...
- **複合インデックス `[client_id, used]`**：未使用コードの検索を高速化するため。
...
- **複合インデックス `[user_id, client_id]`**：ユーザー・クライアント別のトークン検索を高速化。トークン失効（logout）時の一括削除にも利用。
...
- **`user_id` + `client_id` の複合 UNIQUE**：同一ユーザー・同一クライアントの同意は 1 レコードで管理する。スコープが変更された場合は更新（UPSERT）を行う。
