...
- **8 分割の理由**：`feature/project-setup` 単体では「ディレクトリ作成から Rails 雛形、Vue 雛形、Docker 設定、品質ツール設定まで」含めて差分が巨大になり、1 つの PR でレビュー負荷が高くなる。レビュアーがインフラ知識とフロントエンド知識の両方を同時に必要とするとボトルネックが発生するため、専門性に応じてレビュアーを分けられる粒度に分割した。
- **`init` を親とする理由**：全ブランチが共通して「ディレクトリが存在すること」を前提とするため。`init` なしに並行してブランチを切ると、各ブランチで同じ空ディレクトリを作成する重複コミットが発生し、マージ時に競合する可能性がある。
- **並行マージ戦略**：`init` 以降の 8 ブランチは互いに依存しない（例：`docker` と `frontend` は独立）。並行してレビュー・マージすることで、Phase 1 の総所要時間を最短化できる。
