...
簡潔に言うと、**同じタイミングで書き換えるデータが1つのDBだけだから**です。

##### 直感的な例：銀行のATM

- **分散トランザクションが必要なケース**：Aさんの口座から引き落とし **＆** Bさんの口座に振込を**同時に**行う
  - どちらか一方だけ成功すると大事故
  - 2つのDBを同じトランザクションで囲む必要がある

- **今回のケース（不要な理由）**：ATMで残高を**確認**してから、自分の通帳に**記帳**する
  - 残高確認 = 読み取り（`idp-user` への問い合わせ）
  - 記帳 = 書き込み（`idp-auth` DB への認可コード保存）
  - 読み取りと書き込みは独立。確認が古くても別に死なない。
...
[ブラウザ] → 認可エンドポイントを叩く
    ↓
[idp-auth] → 認可コードを生成してDBに保存（★ここが唯一の書き込み）
    ↓
[idp-auth] → idp-userに「このユーザーの認証履歴を教えて」（読み取りのみ）
    ↓
[idp-auth] → idp-clientに「このclient_idの設定を教えて」（読み取りのみ）
    ↓
[ブラウザ] ← 認可コードを返す
```

**書き換えが発生するのは `idp-auth DB` だけ**。他のサービスへの問い合わせは全部「読み取り」なので、トランザクションで縛る対象が存在しません。
...
##### 分散トランザクションが必要になる境界条件

分散トランザクションは、**1つのリクエストや処理の中で複数DBに対し「すべて成功するか、すべて失敗するか」の強い整合性を保証する必要がある時**だけ必要になる。

以下は、SSOシステムをマイクロサービス化した際に実際に遭遇する「複数DBを同時に書き換えたくなる」シナリオと、その不整合リスクの具体例である。

###### ケース1：認可コード発行＋認証履歴の同時更新

**シナリオ**：ユーザーがログインに成功し、認可エンドポイントが認可コードを発行する瞬間に、同時に「いつ・誰が・どのクライアントに対して認証したか」を監査ログとして保存したい。

| 項目 | 内容 |
|---|---|
| **書き込み対象** | `idp-auth`（認可コード保存）+ `idp-user`（認証履歴） |
| **不整合のリスク** | 認可コードは発行されたが認証履歴が残らない → 監査で「コードは誰に発行されたか」が追えなくなる。逆に履歴だけ残ってコードが発行されないと、ユーザーはログイン済みと表示されるのにアクセストークンが取得できない |
| **今回の回避策** | 認証履歴は `idp-user` 内部で独立して非同期保存。`idp-auth` からは認証APIで「この認証情報でOK？」と問い合わせるだけ（読み取り） |

###### ケース2：アクセストークン発行＋リフレッシュトークン管理

**シナリオ**：トークンエンドポイントで、認可コードを交換する際に「リフレッシュトークン回転（Rotation）」を実装している。古いリフレッシュトークンを失効させ、新しいアクセストークンと新しいリフレッシュトークンを同時に発行する。

| 項目 | 内容 |
|---|---|
| **書き込み対象** | `idp-auth`（アクセストークン発行）+ `idp-auth`（古いリフレッシュトークン失効）+ `idp-user`（ログイン履歴：「リフレッシュによる再発行」記録） |
| **不整合のリスク** | 古いリフレッシュトークンが失効せず、新しいリフレッシュトークンも発行される → 同じ認可フローから2つの有効なリフレッシュトークンが存在し、リプレイ攻撃のリスクが増大。または新トークンが発行できず古いトークンだけ失効 → ユーザーが永続的にログインできなくなる（リフレッシュ不能） |
| **今回の回避策** | リフレッシュトークンの管理は `idp-auth` 内部で完結。`idp-user` への「リフレッシュによる再発行」記録は非同期イベントで後から送る。最悪記録が失われてもセキュリティは破綻しない |

###### ケース3：同意情報の変更＋クライアント設定の変更

**シナリオ**：管理画面で「このクライアントは `email` スコープを要求できる」という設定を変更した直後に、ユーザーがそのクライアントに対して同意画面を表示・更新する。同意画面では「`email` と `profile` の提供に同意します」とユーザーがクリックした。

| 項目 | 内容 |
|---|---|
| **書き込み対象** | `idp-client`（クライアントの許可スコープ変更）+ `idp-auth`（ユーザーの同意記録更新） |
| **不整合のリスク** | クライアント設定では `email` が許可されたが、同意記録では古い設定に基づいて `profile` しか同意されていない。結果、クライアントは `email` を要求するがIdPは「ユーザーが同意していない」と拒否し、OAuthフローが失敗する。ユーザー視点では「なぜかログインできない」という謎の障害になる |
| **今回の回避策** | クライアント設定の変更は管理画面からの独立した操作。同意画面表示時に `idp-client` から最新の許可スコープを読み取り（読み取りのみ）、`idp-auth` 内で同意記録を書き込む。書き換えは1つのDBだけ |

###### ケース4：パスワード変更＋全セッション・リフレッシュトークンの失効（グローバルサインアウト）

**シナリオ**：ユーザーが「パスワードを変更したので、他の端末からも強制的にログアウトしてほしい」と要求。これはセキュリティ侵害を受けた後の一般的な対応。

| 項目 | 内容 |
|---|---|
| **書き込み対象** | `idp-user`（パスワードハッシュ更新）+ `idp-auth`（全リフレッシュトークン失効）+ `idp-auth`（全セッションCookie無効化） |
| **不整合のリスク** | パスワードだけ変更されてリフレッシュトークンが失効しない → 攻撃者は古いリフレッシュトークンで永続的にアクセストークンを再発行し続けられる。逆にリフレッシュトークンだけ失効してパスワードが変更されない → 攻撃者は新しいパスワードで再ログインできてしまう |
| **今回の回避策** | 個人開発/モジュラーモノリスでは「パスワード変更」と「セッション一括失効」は**同期的に同じDBトランザクション内**で実行される。独立DB設計にしないか、あるいはパスワード変更後に非同期イベントで「全セッション失効」を発行し、数秒の遅延を許容する |

###### ケース5：ユーザー退会＋全クライアントでの連携解除（GDPR・データ削除）

**シナリオ**：GDPRの「忘れられる権利」対応や、ユーザー自身の退会操作。IdP上のユーザーデータだけでなく、連携先SaaS（Slack、Salesforce等）でのユーザープロビジョニングも解除する必要がある。

| 項目 | 内容 |
|---|---|
| **書き込み対象** | `idp-user`（ユーザーデータ削除）+ `idp-auth`（全トークン・同意記録削除）+ 各連携先SaaS（プロビジョニング解除API呼び出し） |
| **不整合のリスク** | IdP上ではユーザーが削除されたが、SaaS側では連携が残っている → 「存在しないユーザー」としてSaaSが振る舞い、請求やアクセス権のゴミが残る。またはSaaS側だけ解除されてIdP側が残っている → ユーザーは「もう連携解除したはずなのに」という体験になる |
| **今回の回避策** | 個人開発レベルではこの規模の要件は想定外。もし必要なら「アウトボックスパターン」で `idp-user` の退会処理と同じトランザクションで「解除イベント」をDBに書き、非同期ワーカーが各SaaSに配送する。SaaS連携の失敗はリトライで対処し、分散TXは使わない |

###### ケース6：クライアント登録＋承認ワークフロー＋初期シークレット発行

**シナリオ**：エンジニアが「新しいアプリを登録したい」と申請。管理者が承認するまで有効にならず、承認と同時に `client_secret` が発行され、監査ログにも残る。

| 項目 | 内容 |
|---|---|
| **書き込み対象** | `idp-client`（クライアント情報保存）+ `idp-client`（承認ステータス更新）+ `idp-auth`（初期シークレット発行）+ 監査DB（承認者・承認時刻記録） |
| **不整合のリスク** | 承認されたが `client_secret` が発行されない → アプリが動かない。`client_secret` は発行されたが承認ステータスが更新されない → 未承認のままAPIが使えてしまう。監査ログが残らない → 誰がいつ承認したか追えなくなる |
| **今回の回避策** | 個人開発では承認ワークフロー自体を省略し、管理者が直接登録する。またはクライアント登録は `idp-client` 内の単一トランザクションで完結させ、監査ログは非同期で送る |

---

**これらの共通点**：「1つのユーザー操作や管理操作から、**複数のドメインにまたがる永続データを同時に書き換える必要がある**」時に分散トランザクションが必要になる。SSOでは認証・認可・ユーザー・クライアント・同意・セッション・監査が絡み合うため、この境界を誤って設計すると自然と「複数DB同時更新」が生じやすい。

今回の設計では、これらを以下のように回避している：

- 認証履歴の更新は `idp-user` 内部で独立して行う（`idp-auth` からは読み取りのみ）
- リフレッシュトークン・アクセストークンの管理は `idp-auth` 内部で完結する（別DBに持たない）
- クライアント設定の変更は管理画面からの独立した操作であり、認可コード発行と同時には起こらない
- パスワード変更＋セッション失効は、単一DB内で同期的に実行されるか、非同期イベントで許容する
- 退会やSaaS連携解除は個人開発のスコープ外とし、必要になった時点でイベント駆動アーキテクチャを導入する
- 承認ワークフローは省略するか、単一モジュール内で完結させる

##### 分散トランザクションの実現手段と個人開発での問題

仮に上記のようなケースが出た時の選択肢：

| 方式 | 仕組み | 個人開発での問題 |
|---|---|---|
| **2PC（Two-Phase Commit）** | コーディネーターが全DBに「準備OK？」→「コミット実行」。全員がコミットするまでロックが掛かる | 遅延が大きい。コーディネーターが落ちると復旧手続きが複雑 |
| **Sagaパターン** | 各サービスが独立にコミット。失敗したら逆順で補償処理（取消し）を実行 | 補償処理の設計が難しい。途中でクラッシュした Saga の状態復元が地獄 |
| **TCC（Try-Confirm-Cancel）** | 予約→確定→取消の3段階。各サービスが予約API、確定API、取消APIを持つ | 各サービスに3倍のAPI実装が必要。設計・テストコストが爆発的 |

個人開発では、これらを正しく実装・運用し続けるコストが OIDC プロトコル自体の学習コストを上回る。だからこそ「書き込みは自DB内だけに閉じる」という設計を選ぶ。
